...
export default function RecipeDetails({ user }) {
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      ...
      setLoading(false); // End loading after we set the recipe
    };
    fetchRecipe();
  }, []);
  ...
  const formattedSummary = convertHtmlToText(recipe.summary).replace(
    /\.\s/g,
    ".\n\n"
  );
                ...
                <ul className="ingredients-box">
                  {recipe.extendedIngredients?.map((ingredient) => (
                    <li
                      key={ingredient.id}
                      className={
                        checkedIngredients[ingredient.id] ? "checked" : ""
                      }
                    >
                      <label>
                        <input
